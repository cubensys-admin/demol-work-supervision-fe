...
import { getResources } from '@/entities/resource/api/getResources';
import type { ResourceSummary } from '@/entities/resource/model/types';
...
export function ResourceListCard({ isManageMode = false, onDelete }: ResourceListCardProps) {
  ...
  const fetchResources = useCallback(async (pageNum: number = 0) => {
    setIsLoading(true);
    try {
      ...
      console.error(error);
      toast.error('자료실 목록을 불러오는 중 오류가 발생했습니다.');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRowClick = (id: number) => {
    const path = isManageMode ? `/resources-manage/${id}` : `/archives/${id}`;
    router.push(path);
  };
  ...
  return (
    <div className="rounded-[20px] bg-white p-6 shadow-[2px_4px_20px_rgba(0,0,0,0.05)]">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-[18px] font-bold">서식 및 자료실</h1>
        <button
          type="button"
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {showManageButtons && (
            <Button type="button" onClick={() => router.push('/resources-manage/create')}>
              새 자료 등록
            </Button>
          )}
        </div>
      ...
      <div className="overflow-x-auto">
          <div className="w-full overflow-hidden">
            <table className="min-w-full border-collapse text-sm">
              <caption className="sr-only">자료 목록</caption>
              ...
              <tbody className="divide-y divide-border-neutral/70 text-[14px] text-heading">
                {isLoading ? (
                  <tr>
                    <td colSpan={showManageButtons ? 6 : 5} className="px-4 py-12 text-center text-secondary">
                      불러오는 중...
                    </td>
                  </tr>
                ) : items.length === 0 ? (
                  ...
                  items.map((item) => {
                    return (
                      <tr key={item.id} className="bg-white hover:bg-gray-50">
                        <td className="px-5 py-4 align-middle text-center text-secondary">{item.id}</td>
                          ...
                          <td className="px-5 py-4 align-middle">
                            <div className="flex flex-wrap items-center justify-center gap-2">
                              <Button
                                ...
                                size="sm"
                                variant="secondary"
                                onClick={() => router.push(`/resources-manage/${item.id}/edit`)}
                              >
                                수정
                              </Button>
