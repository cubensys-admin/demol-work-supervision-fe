...
import { getDistrictDemolitionRequests, cancelDemolitionRequest } from '@/entities/demolition/api';
import type {
  DemolitionRequestStatus,
  DemolitionRequestSummary,
  DemolitionRequestType,
} from '@/entities/demolition/model/types';
import { getDistrictDemolitionStatusLabel, getDistrictDemolitionStatusBadge, getDemolitionTypeLabel } from '@/entities/demolition/model/status';
...
type DistrictStatusGroup = 'ALL' | 'VERIFICATION' | 'ASSIGNED' | 'COMPLETED' | 'CANCELLED';

const DISTRICT_STATUS_GROUP_MAP: Record<Exclude<DistrictStatusGroup, 'ALL'>, DemolitionRequestStatus[]> = {
  VERIFICATION: ['INITIAL_REQUEST', 'INITIAL_REJECTED', 'RE_REQUEST', 'VERIFICATION_REQUESTED', 'VERIFICATION_COMPLETED', 'VERIFICATION_REJECTED'],
  ASSIGNED: ['RECOMMENDATION_COMPLETED', 'SUPERVISOR_ASSIGNED'],
...
/**
 * District Office Demolition Request List
 * Shows demolition requests for district office role with management actions
 */
export function DistrictDemolitionRequestList() {
  ...
  const [statusFilter, setStatusFilter] = useState<DistrictStatusGroup>('ALL');
  ...
  const statusOptions: { value: DistrictStatusGroup; label: string }[] = [
    { value: 'ALL', label: '전체 상태' },
    { value: 'VERIFICATION', label: '검증요청' },
    { value: 'ASSIGNED', label: '감리자 지정' },
    { value: 'COMPLETED', label: '감리 완료' },
  ...
  const loadRequests = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await getDistrictDemolitionRequests({
        page: currentPage,
        size: pageSize,
  ...
  const handleApplyFilters = (event: FormEvent) => {
    ...
    setAppliedFilters({
      statuses: statusFilter === 'ALL' ? undefined : DISTRICT_STATUS_GROUP_MAP[statusFilter],
      requestType: requestTypeFilter === 'ALL' ? undefined : requestTypeFilter,
      region: regionFilter === 'ALL' ? undefined : regionFilter,
      zone: zoneFilter === 'ALL' ? undefined : zoneFilter,
      periodNumber: periodNumberFilter ? Number(periodNumberFilter) : undefined,
    });
    setCurrentPage(0);
  };
  ...
  const handleRowClick = (id: number) => {
    router.push(`/district/demolition/${id}`);
  };

  const handleWithdrawal = async (id: number) => {
    if (!window.confirm('해당 요청을 취하하시겠습니까?')) return;

    try {
      await cancelDemolitionRequest(id);
      toast.success('요청이 취하되었습니다.');
      loadRequests();
    } catch (error) {
      console.error(error);
      toast.error('요청 취하에 실패했습니다.');
    }
  };
  ...
  return (
    <div className="rounded-[20px] bg-white p-6 shadow-[2px_4px_20px_rgba(0,0,0,0.05)]">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-[18px] font-bold">감리자 의뢰내역</h1>
        <button
          type="button"
      ...
      >
        ...
        <div className="flex flex-col">
          {/* First Row: 요청유형 (라디오) | 상태 (셀렉트) | 지역 (셀렉트) */}
          ...
          {/* Second Row: 권역 (셀렉트) | 기수 (입력) | 조회 | 초기화 */}
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => router.push('/district/demolition/request')}>
            새 요청 등록
          </Button>
        </div>
      </div>
          ...
          <table className="min-w-full border-collapse text-sm">
            ...
            <thead className="bg-[#EDF6FF]">
              <tr className="h-12">
                ...
                <th className="px-5 text-left text-[14px] font-semibold text-[#010101]" style={{ minWidth: '100px' }}>요청번호</th>
                ...
                <th className="px-5 text-center text-[14px] font-semibold text-[#010101]" style={{ minWidth: '130px' }}>검증요청일</th>
                <th className="px-5 text-center text-[14px] font-semibold text-[#010101]" style={{ minWidth: '130px' }}>검증완료일</th>
                <th className="px-5 text-left text-[14px] font-semibold text-[#010101]" style={{ minWidth: '100px' }}>감리자</th>
              ...
              {!isLoading &&
                requests.map((request) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 align-middle text-center">
                      <span className={getDistrictDemolitionStatusBadge(request.status)} style={{ display: 'inline-block', width: '88px', textAlign: 'center', padding: '8px 12px', borderRadius: '4px' }}>
                        {getDistrictDemolitionStatusLabel(request.status)}
                      </span>
                    </td>
                    <td className="px-5 py-4 align-middle text-center text-secondary">
                    ...
                    <td className="px-5 py-4 align-middle text-center" onClick={(e) => e.stopPropagation()}>
                      {request.status === 'SUPERVISOR_ASSIGNED' && (
                        <Button
                          size="sm"
                          variant="secondary"
                          onClick={() => handleWithdrawal(request.id)}
                        >
                          취하 신청
                        </Button>
                      )}
