...
// 감리자 추천 페이지에서 보여줄 상태 (접수, 재지정 요청, 검토요청, 검토완료)
const VERIFICATION_STATUS_ALLOWED: DemolitionRequestStatus[] = [
  'INITIAL_REQUEST',
...
/**
 * City Hall Verification List
 * Shows demolition requests with VERIFICATION_REQUESTED, VERIFICATION_COMPLETED, VERIFICATION_REJECTED statuses
 */
export function CityVerificationList() {
  ...
  const [appliedFilters, setAppliedFilters] = useState<AppliedFilters>({ statuses: VERIFICATION_STATUS_ALLOWED });
  const [isFilterExpanded, setIsFilterExpanded] = useState(true);

  const statusOptions: { value: 'ALL' | DemolitionRequestStatus; label: string }[] = [
    { value: 'ALL', label: '전체 상태' },
    { value: 'INITIAL_REQUEST', label: '접수' },
    { value: 'VERIFICATION_REJECTED', label: '재지정 요청' },
    { value: 'VERIFICATION_REQUESTED', label: '검토요청' },
    { value: 'VERIFICATION_COMPLETED', label: '검토완료' },
  ];
  ...
  const handleApplyFilters = (event: FormEvent) => {
    ...
    const filters: AppliedFilters = {
      statuses: statusFilter === 'ALL' ? VERIFICATION_STATUS_ALLOWED : [statusFilter],
  ...
  const handleReset = () => {
    ...
    setSearchKeyword('');
    setAppliedFilters({ statuses: VERIFICATION_STATUS_ALLOWED });
    setCurrentPage(0);
  };
  ...
  return (
    <div className="rounded-[20px] bg-white p-6 shadow-[2px_4px_20px_rgba(0,0,0,0.05)]">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-[18px] font-bold">감리자 추천</h1>
        <button
          type="button"
