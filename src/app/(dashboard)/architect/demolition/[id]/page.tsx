...
import { getArchitectDemolitionRequestById } from '@/entities/demolition/api';
...
export default function ArchitectDemolitionDetailPage() {
  ...
  useEffect(() => {
    if (role && role !== 'ARCHITECT_SOCIETY') {
      router.replace('/');
      return;
    }

    if (!id || Number.isNaN(id)) {
      router.replace('/architect/demolition/status');
      return;
    }
    ...
    const loadRequest = async () => {
      setIsLoading(true);
      try {
        const data = await getArchitectDemolitionRequestById(id);
        setRequest(data);
      } catch (error) {
        console.error(error);
